      ...
      <div style={{ display: 'flex', gap: 3 }}>
        {tiers.map((tier, idx) => (
          <div key={tier} style={{
            ...
            background: idx <= planIndex ? C.orange : 'transparent',
            border: `1.5px solid ${idx <= planIndex ? C.orange : (darkMode ? 'rgba(255,255,255,0.2)' : 'rgba(0,31,84,0.18)')}`
          }} />
        ))}
      </div>
    )
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div onClick={() => setShowLeaderboard(true)} style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{
              ...
              background: C.orange, color: '#fff', borderRadius: 50, padding: '7px 16px',
          ...
          <div style={{
            ...
            <svg width="26" height="26" viewBox="0 0 24 24" fill={C.orange}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
          <div style={{ background: tk.card, borderRadius: 16, padding: '14px 8px 12px', textAlign: 'center', boxShadow: tk.iconShadow, border: `1.5px solid ${tk.cardBorder}` }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: C.orange }}>0</div>
          </div>
          <div style={{ background: tk.card, borderRadius: 16, padding: '14px 8px 12px', textAlign: 'center', boxShadow: tk.iconShadow, border: `1.5px solid ${tk.cardBorder}` }}>
            <div style={{ fontSize: 9, fontWeight: 800, color: tk.textMuted, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 8 }}>Active Invites</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: C.orange }}>0</div>
          </div>
          <div style={{ background: tk.card, borderRadius: 16, padding: '14px 8px 12px', textAlign: 'center', boxShadow: tk.iconShadow, border: `1.5px solid ${tk.cardBorder}` }}>
            <div style={{ fontSize: 9, fontWeight: 800, color: tk.textMuted, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 8 }}>Active VIP</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: C.orange }}>0</div>
          </div>
          <div style={{ background: tk.card, borderRadius: 16, padding: '14px 8px 12px', textAlign: 'center', boxShadow: tk.iconShadow, border: `1.5px solid ${tk.cardBorder}` }}>
            <div style={{ fontSize: 9, fontWeight: 800, color: tk.textMuted, letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 8 }}>Total Earned</div>
            <div style={{ fontSize: 17, fontWeight: 900, color: C.orange }}>$0.00</div>
          </div>
        </div>
        ...
        <div style={{ background: tk.card, borderRadius: 18, padding: '4px 4px 4px 18px', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 12, boxShadow: tk.iconShadow, border: `1px solid ${tk.cardBorder}` }}>
          <span style={{ flex: 1, fontSize: 12, color: tk.textMuted, fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {referralLink}
          </span>
          <button onClick={copyLink} style={{
            background: C.orange, color: '#fff', borderRadius: 14, padding: '11px 22px',
        ...
        <button onClick={handleShare} style={{
          width: '100%', background: tk.card, border: `1.5px solid ${tk.cardBorder}`,
          borderRadius: 18, padding: 14, marginBottom: 20,
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
          fontSize: 14, fontWeight: 800, color: tk.text, cursor: 'pointer',
          boxShadow: tk.iconShadow,
        }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={tk.text} strokeWidth="2.2">
